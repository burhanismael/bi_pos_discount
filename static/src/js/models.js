...
exports.Orderline = Backbone.Model.extend({
    ...
    set_discount: function(discount){
        ...
        //my customization Lines(1939-1939)
        debugger;
        if (this.pos.config.discount_type === 'percentage'){
            ...
            debugger;
            var amount = this.get_unit_price() * this.get_quantity()
            var disc = Math.min(Math.max(parsed_discount || 0, 0), isNaN(amount) ? parsed_discount : amount);
        }
        debugger;
        this.discount = isNaN(disc) ? 0 : disc;
        debugger;
        this.discountStr = isNaN(disc) ? '0' : '' + disc;
    ...
    get_base_price:    function(){
        ...
        //my customization Lines(2236-2239)
    ...
    get_display_price_one: function(){
        ...
        if (this.pos.config.iface_tax_included !== 'total') {
            //my customization Lines(2246-2249)
            if (this.pos.config.discount_type === 'percentage')
...
//                return round_pr(price_unit - this.get_discount(), rounding);
            ...
            //my customization Lines(2264-2267)
            if (typeof this !== 'undefined'){
                if (this.pos.config.discount_type === 'fixed')
    ...
    get_all_prices: function(){
        var self = this;
        //my customization Lines(2529-2532)
        var price_unit = this.get_unit_price() * (1.0 - (this.get_discount() / 100.0));
        if (typeof this !== 'undefined'){
            if (this.pos.config.discount_type === 'fixed')
                price_unit = ((this.get_unit_price() * this.get_quantity()) - this.get_discount()) / this.get_quantity();
        }
        var taxtotal = 0;
...
exports.Order = Backbone.Model.extend({
    ...
    get_total_discount: function() {
        return round_pr(this.orderlines.reduce((function(sum, orderLine) {
            //my customization Lines(3468-3473)
            if (typeof this !== 'undefined'){
                if (this.pos.config.discount_type === 'fixed')
                    sum += (orderLine.get_unit_price() * orderLine.get_quantity()) - orderLine.get_discount();
                else
